...
if [ "$JPEGXL" = true ]; then
 export VIPSJPEGXL+="-Djpeg-xl=enabled"
else
 export VIPSJPEGXL+="-Djpeg-xl=disabled"
fi

if [ "$OPENJPEG" = true ]; then
  export VIPSOPENJPEG+="-Dopenjpeg=enabled"
  export HEIFOPENJPEG+="-DWITH_OpenJPEG_ENCODER=ON -DWITH_OpenJPEG_DECODER=ON"
else
  export VIPSOPENJPEG+="-Dopenjpeg=disabled"
  export HEIFOPENJPEG+=""
fi
...
CFLAGS="${CFLAGS} -O3 -DLIBJPEG_TURBO_VERSION_NUMBER=3001001 -I${TARGET}/include" CXXFLAGS="${CXXFLAGS} -O3 -DLIBJPEG_TURBO_VERSION_NUMBER=3001001 -I${TARGET}/include" cmake -G"Unix Makefiles" \
  -DCMAKE_TOOLCHAIN_FILE=${ROOT}/Toolchain.cmake -DCMAKE_INSTALL_PREFIX=${TARGET} -DLIBJPEG_TURBO_VERSION_NUMBER=3001001 -DCMAKE_INSTALL_LIBDIR=lib ${HEIFOPENJPEG} -DCMAKE_BUILD_TYPE=Release  \
  -DCMAKE_C_FLAGS="-O3 -DLIBJPEG_TURBO_VERSION_NUMBER=3001001 -I${TARGET}/include" \
  -DCMAKE_CXX_FLAGS="-O3 -DLIBJPEG_TURBO_VERSION_NUMBER=3001001 -I${TARGET}/include" \
...
CFLAGS="${CFLAGS} -O3" CXXFLAGS="${CXXFLAGS} -O3" meson setup _build --default-library=shared --buildtype=release --strip --prefix=${TARGET} ${MESON} \
  -Ddeprecated=false -Dexamples=false -Dintrospection=disabled -Dmodules=disabled -Dcfitsio=disabled -Dfftw=disabled ${VIPSJPEGXL} \
  -Dmagick=disabled -Dmatio=disabled -Dnifti=disabled -Dopenexr=disabled ${VIPSOPENJPEG} -Dopenslide=disabled \
