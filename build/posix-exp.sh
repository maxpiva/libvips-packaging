...
if [ "$HEVC" = true ]; then
  ...
  export HEIFX265+="-DWITH_LIBDE265=0 -DWITH_X265=0"
fi
...
CFLAGS="${CFLAGS} -O3" CXXFLAGS="${CXXFLAGS} -O3 -I${TARGET}/include" cmake -G"Unix Makefiles" \
  ...
  -DCMAKE_INSTALL_LIBDIR=lib -DCMAKE_BUILD_TYPE=Release \ -DWITH_OpenJPEG_ENCODER=ON -DWITH_OpenJPEG_DECODER=ON
  -DBUILD_SHARED_LIBS=FALSE -DBUILD_TESTING=0 -DENABLE_PLUGIN_LOADING=0 -DWITH_EXAMPLES=0 -DCMAKE_CXX_FLAGS="$(CXXFLAGS) -DCMAKE_INSTALL_PREFIX=${TARGET} -DCMAKE_PREFIX_PATH=${TARGET} -DHAVE_JPEG_WRITE_ICC_PROFILE -DWITH_JPEG=ON -DWITH_LIBJPEG_TURBO=ON" -DWITH_JPEG=ON ${HEIFX265}
make install/strip
if [ "$PLATFORM" == "linux-arm" ]; then
