...
CFLAGS="${CFLAGS} -O3 -I${TARGET}/include" CXXFLAGS="${CXXFLAGS} -O3 -I${TARGET}/include" cmake -G"Unix Makefiles" \
  ...
  -DBUILD_SHARED_LIBS=FALSE -DBUILD_TESTING=0 -DENABLE_PLUGIN_LOADING=0 -DWITH_EXAMPLES=0 -DCMAKE_CXX_FLAGS="$(CXXFLAGS) -DWITH_JPEG=ON -DWITH_LIBJPEG_TURBO=ON" -DWITH_JPEG=ON -DWITH_LIBJPEG_TURBO=ON ${HEIFX265}
make install/strip
if [ "$PLATFORM" == "linux-arm" ]; then
